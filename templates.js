...
function getTradeResult(t) {
  ...
  // Risk = dollar value risked (entry→SL * size), fallback to |pnl| if no SL
  const slDiff = t.sl ? Math.abs(t.entry - t.sl) : 0;
  const risk   = slDiff > 0 ? slDiff * (t.size || 1) * 100 : Math.abs(t.pnl);
...
function initSettingsUI() {
  ...
  if (tzSel && tzSel.options.length === 0) {
    ...
    const userTZ = s.timezone || Intl.DateTimeFormat().resolvedOptions().timeZone;
    tzList.forEach(tz => {
  ...
  setVal('setting-timezone',     s.timezone);
  ...
  // Start clock
  startTzClock(s.timezone);
}
function applyTimezone() {
  const sel = $('setting-timezone');
  if (!sel) return;
  saveSetting('timezone', sel.value);
...
function setThemeMode(mode) {
  ...
  const modeNames = { dark: '🌑 Midnight', medium: '✦ Aura', light: '☀️ Dawn' };
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  applyAccentColor(s.accentColor || '#6b1fd4');
  if (s.accentColor) {
