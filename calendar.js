...
function showAddAccountInput(){$('f-account-add-btn').style.display='none';$('f-account-add-input-row').style.display='flex';$('f-account-custom-input').focus();}
...
function clearDropdown(name, e) {
  ...
  lbl.textContent = name==='model' ? 'Select model...' : 'Select...';
...
document.addEventListener('click', function(e) {
  ['setup','session','account'].forEach(n => {
    const dd=$('f-'+n+'-dd');
    const tr=$('f-'+n+'-trigger');
    if(dd && dd.style.display!=='none' && !dd.contains(e.target) && e.target!==tr && !tr?.contains(e.target)){
...
(function(){
  ...
  function clkDraw() {
    ...
    if(clkMode==='hr'){
      for(let i=1;i<=12;i++){
        ...
        ctx.fillStyle=(i===clkHour%12||(clkHour===0&&i===12))?'#fff':'rgba(160,168,190,.7)';
      ...
      for(let i=0;i<12;i++){
        ...
        ctx.fillStyle=Math.abs(clkMin-m)<3?'#fff':'rgba(160,168,190,.7)';
  ...
  window.clkConfirm=function(){
    ...
    const display12h=format24to12Hour(val24h);
    $('f-time').value=display12h;
...
function initDrawdownChart(period) {
  ...
  trades.forEach(t => {
    ...
    labels.push(t.date + (t.time ? ' '+format24to12Hour(t.time) : ''));
        ...
        tooltip: {
          ...
          external(context) {
            ...
            $('dd-tt-date').textContent = trade
              ? (trade.date+(trade.time?' · '+format24to12Hour(trade.time):'')+(trade.symbol?' · '+trade.symbol:''))
              : 'Starting balance';
...
function initRMultiple() {
  ...
  c.lineWidth = 1;
  c.beginPath(); c.moveTo(midX, PAD_T - 8); c.lineTo(midX, PAD_T + rs.length * ROW_H + 4); c.stroke();
...
function initStreaks() {
  ...
  timeline.innerHTML = trades.map((t, i) => {
    ...
    const tip    = `${t.date}${t.time?' '+format24to12Hour(t.time):''} · ${t.symbol||''} · ${pnlStr}`;
