...
TRADES = TRADES.map(t => {
  ...
  if (!isImported || !t.time) return t;
  const nextSession = typeof inferSessionFromTradingTime === 'function'
    ? inferSessionFromTradingTime(t.time)
    : (typeof inferSessionFromESTTime === 'function' ? inferSessionFromESTTime(t.time) : t.session);
  if (!nextSession || nextSession === t.session) return t;
...
let dirFilter = 'all';
window._tradeLogDrilldownActive = false;

function clearTradeLogTransientFilters(skipApply) {
  ...
  $$('.fpill[data-val="all"]').forEach(p => p.classList.add('active'));

  const symLabel = $('fp-symbol-label');
  if (symLabel) { symLabel.textContent = 'All Symbols'; symLabel.style.color = ''; }
  const sessionLabel = $('fp-session-label');
  if (sessionLabel) { sessionLabel.textContent = 'All Sessions'; sessionLabel.style.color = ''; }
  const fromLabel = $('fp-from-label');
  const toLabel = $('fp-to-label');
  const fromDisplay = $('fp-date-from-display');
  const toDisplay = $('fp-date-to-display');
  if (fromLabel) fromLabel.textContent = 'From';
  if (toLabel) toLabel.textContent = 'To';
  if (fromDisplay) fromDisplay.style.color = 'var(--text3)';
  if (toDisplay) toDisplay.style.color = 'var(--text3)';

  if (!skipApply && typeof applyFilters === 'function') applyFilters();
}

function recoverEmptyTradeLogIfStale() {
  const tradeLogActive = document.querySelector('#view-tradelog.view.active');
  if (!tradeLogActive) return;
  if (!Array.isArray(TRADES) || TRADES.length === 0) return;
  if (Array.isArray(filteredTrades) && filteredTrades.length > 0) return;
  // Only auto-clear if a transient filter is likely causing the empty state
  const hasTransientFilter = window._tradeLogDrilldownActive || calFromVal || calToVal ||
    window._symbolQuickFilter || window._sessionQuickFilter || window._streakIdFilter;
  if (!hasTransientFilter) {
    // No transient filter active — just re-apply to refresh in case filteredTrades is stale
    if (typeof applyFilters === 'function') applyFilters();
    return;
  }
  clearTradeLogTransientFilters(false);
  if (typeof showToast === 'function') {
    showToast('Recovered trade log from stale filter state', 'info', '', 1800);
  }
}

document.addEventListener('visibilitychange', () => {
  if (!document.hidden) recoverEmptyTradeLogIfStale();
});

window.addEventListener('focus', () => {
  recoverEmptyTradeLogIfStale();
});
...
function switchView(el) {
  try {
    ...
    if (currentViewId === 'view-tradelog' && v !== 'tradelog' && window._tradeLogDrilldownActive) {
      clearTradeLogTransientFilters(true);
    }
    ...
    try {
      if(v==='tradelog') {
        ...
        // Always re-apply filters when entering the trade log so filteredTrades is never stale
        if (typeof applyFilters === 'function') applyFilters();
        if (typeof initTlogScrollIndicator === 'function') setTimeout(() => initTlogScrollIndicator(), 50);
        setTimeout(() => recoverEmptyTradeLogIfStale(), 80);
...
function updateTlCards(trades) {
  ...
  const netText = (netPnl >= 0 ? '+$' : '-$') + Math.abs(netPnl).toFixed(2);
  const netClass = 'stat-val ' + (netPnl >= 0 ? 'c-green' : 'c-red');
  const netEl = $('tl-net');
  if (netEl) {
    netEl.textContent = netText;
    netEl.className = netClass;
  }
  const netHoverEl = $('tl-net-hover');
  if (netHoverEl) {
    netHoverEl.textContent = netText;
    netHoverEl.className = netClass;
  }
  const grossEl = $('tl-gross');
  if (grossEl) {
    grossEl.textContent = (grossPnl >= 0 ? '+$' : '-$') + Math.abs(grossPnl).toFixed(2);
    grossEl.style.setProperty('color', grossPnl >= 0 ? 'var(--green)' : 'var(--red)', 'important');
  }
  ...
  if (wrEl) {
    ...
    wrEl.style.setProperty('color', wrColor, 'important');
  }
  ...
  if (pfEl) {
    if (losses.length === 0 && wins.length > 0) {
      pfEl.textContent = '∞';
      pfEl.style.setProperty('color', 'var(--purple)', 'important');
  ...
  if (trades.length > 0) {
    ...
    const worstColor    = worst.pnl >= 0 ? 'var(--green)' : 'var(--red)';
    ...
    if (bestValEl)  { bestValEl.textContent  = _bwM === 'pct' ? bestPctStr  : bestDolStr;  bestValEl.style.setProperty('color', bestColor, 'important'); }
    if (worstValEl) { worstValEl.textContent = _bwM === 'pct' ? worstPctStr : worstDolStr; worstValEl.style.setProperty('color', worstColor, 'important'); }
    if (bestLblEl)  { bestLblEl.textContent  = _bwM === 'pct' ? bestDolStr  : bestPctStr;  bestLblEl.style.setProperty('color', bestColor, 'important'); }
    if (worstLblEl) { worstLblEl.textContent = _bwM === 'pct' ? worstDolStr : worstPctStr; worstLblEl.style.setProperty('color', worstColor, 'important'); }
...
function syncBwToggleButtons(mode) {
  const btnDollar = $('tl-bw-toggle-dollar');
  const btnPct = $('tl-bw-toggle-pct');
  if (btnDollar) {
    btnDollar.style.setProperty('background', mode === 'dollar' ? 'var(--purple)' : '#fff', 'important');
    btnDollar.style.setProperty('color', mode === 'dollar' ? '#fff' : 'var(--purple)', 'important');
    btnDollar.style.boxShadow = mode === 'dollar' ? '0 1px 4px rgba(0,0,0,.35)' : 'none';
  }
  if (btnPct) {
    btnPct.style.setProperty('background', mode === 'pct' ? 'var(--purple)' : '#fff', 'important');
    btnPct.style.setProperty('color', mode === 'pct' ? '#fff' : 'var(--purple)', 'important');
    btnPct.style.boxShadow = mode === 'pct' ? '0 1px 4px rgba(0,0,0,.35)' : 'none';
  }
}

// ── Best & Worst card: $ / % toggle ──
function setBwCardMode(mode) {
  if (window._bwCardMode === mode) {
    syncBwToggleButtons(mode);
    return;
  }
  ...
  setTimeout(function() {
    ...
    if (bestValEl)  { bestValEl.textContent  = mode === 'pct' ? bestValEl.dataset.pct   : bestValEl.dataset.dollar;  bestValEl.style.setProperty('color', bestValEl.dataset.color || '', 'important'); }
    if (worstValEl) { worstValEl.textContent = mode === 'pct' ? worstValEl.dataset.pct  : worstValEl.dataset.dollar; worstValEl.style.setProperty('color', worstValEl.dataset.color || '', 'important'); }
    // Small labels show the OTHER format
    if (bestLblEl)  { bestLblEl.textContent  = mode === 'pct' ? bestLblEl.dataset.dollar  : bestLblEl.dataset.pct;  bestLblEl.style.setProperty('color', bestLblEl.dataset.color || '', 'important'); }
    if (worstLblEl) { worstLblEl.textContent = mode === 'pct' ? worstLblEl.dataset.dollar : worstLblEl.dataset.pct; worstLblEl.style.setProperty('color', worstLblEl.dataset.color || '', 'important'); }
...
function navigateToBwTrade(el) {
  ...
  window._symbolQuickFilter = null;
  window._tradeLogDrilldownActive = true;
...
function renderTrades(trades) {
  ...
  pageTrades.forEach(t => {
    ...
    tr.innerHTML = checkboxCell + `
      ...
      <td><span style="font-family:var(--font-mono);font-size:11.5px">${t.date}</span><br><span style="font-size:10px;color:var(--text3)">${typeof formatTradeTime === 'function' ? formatTradeTime(t.time) : t.time}</span></td>
      <td>
        <div class="sym-cell">
          <div>
...
function showDetail(t, row) {
  ...
  $('detail-body').innerHTML = `
    ...
    <div class="detail-grid">
      <div class="dg-item"><div class="dg-label">Date</div><div class="dg-val mono">${t.date} ${typeof formatTradeTime === 'function' ? formatTradeTime(t.time) : t.time}</div></div>
  ...
  equityChartInstance = new Chart(newCtx, {
    ...
    options: {
      ...
      onClick: (e, elements) => {
        ...
        clearTradeLogTransientFilters(true);
        window._streakIdFilter = [trade.id];
        window._tradeLogDrilldownActive = true;
...
function buildAnAccountBar() {
  ...
  return `
    <div class="acct-selector an-acct-trigger" data-open="false"
      ...
      style="display:flex;align-items:center;gap:8px;background:var(--bg3);border:1px solid var(--border2);border-radius:8px;padding:3px 10px;cursor:pointer;transition:border-color .15s;user-select:none;min-width:200px;min-height:34px;justify-content:space-between"
      onmouseenter="this.style.borderColor='var(--border3)'"
      onmouseleave="if(this.dataset.open!=='true')this.style.borderColor='var(--border2)'">
      <span style="font-size:12.5px;font-weight:500;color:var(--text)">${displayLabel}</span>
      <span style="color:var(--text3);font-size:10px;margin-left:4px;display:flex;align-items:center">
        <svg class="an-acct-chevron" width="10" height="10" viewBox="0 0 12 12" fill="none" style="transition:transform .2s"><path d="M2 4l4 4 4-4" stroke="currentColor" stroke-width="1.6" stroke-linecap="round" stroke-linejoin="round"/></svg>
      </span>
    </div>
    <div class="an-acct-dd" style="display:none;position:absolute;top:calc(100% + 6px);left:0;background:var(--bg2);border:1px solid var(--ac-20);border-radius:12px;min-width:260px;overflow:hidden;z-index:9999;box-shadow:0 12px 40px rgba(0,0,0,.6),0 0 0 1px var(--ac-08)">
      ${options}
    </div>`;
}
...
function syncModelDropdownFromPlaybooks() {
  ...
  list.innerHTML = plays.map(p => {
    ...
    return `<div class="mdl-opt${isSelected ? ' selected' : ''}" onclick="selectModel('${esc}')" style="display:flex;align-items:center;justify-content:space-between;padding:8px 10px 8px 12px;cursor:pointer;transition:background .12s" onmouseenter="this.style.background='var(--ac-10)'" onmouseleave="this.style.background='${isSelected ? 'var(--ac-14)' : 'transparent'}'">
      <div style="display:flex;align-items:center;gap:7px">
        <span style="font-size:13px">${p.emoji || '📋'}</span>
        <span class="mdl-opt-label" style="font-family:var(--font-mono);font-size:12.5px;color:var(--text)">${name}</span>
      </div>
      ${p.avgR ? `<span style="font-family:var(--font-mono);font-size:10.5px;color:var(--green);opacity:.82">${p.avgR}</span>` : ''}
    </div>`;
  }).join('');
}
...
// Session windows (EST, 24h format as minutes from midnight)
const KILL_ZONES = [
  { name: 'Asia',          start: 19 * 60, end: 24 * 60 },
  { name: 'True Day Open', start: 0,       end: 2 * 60 },
  { name: 'London',        start: 2 * 60,  end: 5 * 60 },
  { name: 'NY AM',         start: 7 * 60,  end: 10 * 60 },
  { name: 'London Close',  start: 10 * 60, end: 12 * 60 },
  // End is exclusive, so 16:01 captures up to and including 16:00.
  { name: 'NY PM',         start: 12 * 60, end: 16 * 60 + 1 },
];
...
const SESSION_INSTRUMENTS = {
  ...
  'NY PM': [
    ...
    'USDJPY','USDCAD','USDCHF','EURUSD','GBPUSD',
    'XAUUSD','XAGUSD','GOLD','SILVER',
  ],
  'London Close': [
  ...
  'True Day Open': [
    // Reset hour around daily rollover
    'XAUUSD','XAGUSD','GOLD','SILVER','EURUSD','GBPUSD','USDJPY','USDCAD','USDCHF',
  ],
};
...
function initPlaybook() {
  ...
  if (!plays.length) {
    ...
    <div class="playbook-empty">
      <div class="playbook-empty-badge"><i></i>Entry Models</div>
      <h3>Your playbook vault is empty. Build your first setup and start tracking edge by model.</h3>
      <p>
        Create a playbook for each strategy you trade. As you log trades, this page automatically shows win rate,
        net P&amp;L, and which setups deserve more capital.
      </p>
      <div class="playbook-empty-actions">
        <button class="playbook-empty-btn primary" onclick="openPlaybookModal()">+ Create First Playbook</button>
        <button class="playbook-empty-btn ghost" onclick="goToView('tradelog')">Go to Trade Log</button>
      </div>
      <div class="playbook-empty-tips">
        <div class="playbook-empty-tip">Tag trades with models</div>
        <div class="playbook-empty-tip">Compare setups by win rate</div>
        <div class="playbook-empty-tip">Cut underperforming patterns</div>
      </div>
    </div>`;
  }
}
...
function getPlaybookListWithSample() {
  ...
  // (sample is hidden if user has dismissed/deleted it)
  const userPbs = getPbList();
  if (localStorage.getItem('eq_sample_pb_dismissed') === '1') {
    return userPbs;
  }
...
function deletePlaybook(idx) {
  // Handle deletion of sample playbook (index 0)
  if (idx === 0) {
    localStorage.setItem('eq_sample_pb_dismissed', '1');
    initPlaybook();
    showToast('Sample playbook removed', 'info', '', 1400);
    return;
  }
  // Adjust index for user playbooks (skip the sample at index 0 if visible)
  const sampleVisible = localStorage.getItem('eq_sample_pb_dismissed') !== '1';
  const actualIdx = sampleVisible ? (idx - 1) : idx;
  const list = getPbList();
...
function openModal() {
  ...
  $('f-setup').value = '';
  $('f-model').value = '';
  $('f-model-label').textContent = 'Add entry model';
  ...
  timeInput.value = typeof formatTradeTime === 'function' ? formatTradeTime(timeStr) : format24to12Hour(timeStr);
...
function openEditTradeModal() {
  ...
  $('f-date').value = t.date;
  // Store 24-hour format as data attribute and display the active user format
  const timeInput = $('f-time');
  timeInput.dataset.time24h = t.time;
  timeInput.value = typeof formatTradeTime === 'function' ? formatTradeTime(t.time) : format24to12Hour(t.time);
  ...
  if(ml){ ml.textContent = t.model || 'Add entry model'; ml.style.color = t.model ? 'var(--text)' : 'var(--text3)'; }
  syncModelDropdownFromPlaybooks();
    ...
    if (editTradeId !== null) {
      ...
      if (idx !== -1) {
        TRADES[idx] = { ...TRADES[idx],
          ...
          model: $('f-model').value || '',
          session: $('f-session').value || TRADES[idx].session || (typeof inferSessionFromTradingTime === 'function' ? inferSessionFromTradingTime(storedTime) : (typeof inferSessionFromESTTime === 'function' ? inferSessionFromESTTime(storedTime) : '')),
      ...
      const newTrade = {
        ...
        setup: $('f-setup').value || 'Custom',
        model: $('f-model').value || '',
        session: $('f-session').value || (typeof inferSessionFromTradingTime === 'function' ? inferSessionFromTradingTime(storedTime) : (typeof inferSessionFromESTTime === 'function' ? inferSessionFromESTTime(storedTime) : '')),
...
function sortTable(th) {
  ...
  const sorted = [...filteredTrades].sort((a, b) => {
    ...
    if (col === 'pnl')    { av = a.pnl;   bv = b.pnl; }
      ...
      const aImported = String(a.setup || '').toLowerCase() === 'imported';
      const bImported = String(b.setup || '').toLowerCase() === 'imported';
      const aValidSL = !aImported && Number.isFinite(a.sl) && Number.isFinite(a.entry) && Number.isFinite(a.exit) && ((a.dir === 'long' && a.sl < a.entry) || (a.dir === 'short' && a.sl > a.entry));
      const bValidSL = !bImported && Number.isFinite(b.sl) && Number.isFinite(b.entry) && Number.isFinite(b.exit) && ((b.dir === 'long' && b.sl < b.entry) || (b.dir === 'short' && b.sl > b.entry));
      const aRisk = aValidSL ? Math.abs(a.entry - a.sl) : 0;
      const bRisk = bValidSL ? Math.abs(b.entry - b.sl) : 0;
      const aMove = a.dir === 'short' ? (a.entry - a.exit) : (a.exit - a.entry);
      const bMove = b.dir === 'short' ? (b.entry - b.exit) : (b.exit - b.entry);
      av = aRisk > 0 ? (aMove / aRisk) : Number.NEGATIVE_INFINITY;
      bv = bRisk > 0 ? (bMove / bRisk) : Number.NEGATIVE_INFINITY;
      if (Number.isFinite(av) && Math.abs(av) > 20) av = Number.NEGATIVE_INFINITY;
      if (Number.isFinite(bv) && Math.abs(bv) > 20) bv = Number.NEGATIVE_INFINITY;
    }
...
function firmLogoHTML(firmName, size, radius, customPhoto) {
  size   = size   === undefined ? 36 : size;
  radius = radius === undefined ? 9  : radius;
  if (customPhoto) {
    return '<div style="width:' + size + 'px;height:' + size + 'px;border-radius:' + radius + 'px;overflow:hidden;flex-shrink:0;border:1px solid var(--border);background:var(--bg4)">'
      + '<img src="' + customPhoto + '" alt="Account" style="width:100%;height:100%;object-fit:cover">'
      + '</div>';
  }
...
function renderAllAccountsSection() {
  ...
  wrap.innerHTML = ACCOUNTS.map(a => {
    ...
    // ── Rule calculations ──
    const challengeType = String(a.challengeType || a.phase || '').toLowerCase();
    const hideProfitTarget = challengeType.includes('funded') || challengeType.includes('instant') || Number(a.profitTarget || 0) <= 0;
    ...
    const profitPct = hideProfitTarget ? '0' : Math.min(100, (Math.max(0, netPnl) / (profitLimit || 1)) * 100).toFixed(0);
    ...
    <div class="acct-status-card status-${computedStatus}" style="position:relative">
      <div style="display:flex;align-items:flex-start;justify-content:space-between;gap:12px">
        ...
        <div style="display:flex;align-items:center;gap:12px;flex:1;min-width:0">
          <div style="flex-shrink:0">${firmLogoHTML(a.firm, 42, 10, a.avatar)}</div>
          <div style="display:flex;align-items:center;gap:8px;min-width:0">
            <div style="font-family:var(--font-display);font-size:16px;font-weight:600;white-space:nowrap;overflow:hidden;text-overflow:ellipsis">${a.firm}</div>
            <span style="background:var(--ac-12);color:#a97de8;border:1px solid var(--ac-30);padding:2px 8px;border-radius:5px;font-size:10px;font-family:var(--font-mono);font-weight:500">${formatAccountPhase(a)}</span>
          </div>
        </div>
        <!-- Right: P&L -->
        <div style="text-align:right;flex-shrink:0">
          <div style="font-family:var(--font-display);font-weight:500;font-size:30px;color:${pnlColor}!important;line-height:1">${pnlSign}${fmt(Math.abs(netPnl))}</div>
          <div style="font-family:var(--font-mono);font-size:10px;color:${pnlColor}!important;margin-top:2px;opacity:.8">${pnlSign}${Math.abs(pnlPct)}%</div>
        </div>
      </div>
      ...
      <div style="display:grid;grid-template-columns:repeat(4,1fr);gap:8px;margin-top:16px;padding-top:14px;border-top:1px solid var(--border)">
        ...
        <div>
          ...
          <div style="font-family:var(--font-display);font-weight:700;font-size:13px;color:${wr>=55?'var(--green)':wr>=50?'var(--amber)':'var(--red)'}!important">${wr}%</div>
        </div>
        <div>
      ...
      <!-- Rule bars -->
      <div style="display:grid;grid-template-columns:${hideProfitTarget ? '1fr' : '1fr 1fr'};gap:10px;margin-top:14px">
        <div>
          <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:4px">
            ...
            <span style="font-size:9px;color:var(--red)!important;font-family:var(--font-mono)">${maxLossPct}% used</span>
          </div>
        ...
        <div>
          <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:4px">
            ...
            <span style="font-size:9px;color:var(--green)!important;font-family:var(--font-mono)">${profitPct}% reached</span>
          </div>
          <div class="acct-rule-bar">
  ...
  const fundedCount   = ACCOUNTS.filter(a => /funded/i.test(a.phase)).length;
  const challengeCount = ACCOUNTS.filter(a => !/funded/i.test(a.phase)).length;
  // Store on window so switchCapView can access
  window._portFundedCap    = fundedCap;
  window._portChallengeCap = challengeCap;
  window._portFundedCount  = fundedCount;
  window._portChallengeCount = challengeCount;
  ...
  if (capEl)  { capEl.textContent = '$' + totalCap.toFixed(2); capEl.style.setProperty('color', 'var(--text)', 'important'); }
  // Reset toggle to funded view on re-render
  window._capView = window._capView || 'funded';
  switchCapView(window._capView);
  if (pnlEl)  { pnlEl.textContent = pnlSign + '$' + Math.abs(combinedPnl).toFixed(2); pnlEl.style.setProperty('color', pnlColor, 'important'); }
  if (hlthEl) { hlthEl.textContent = `${active} · ${passed} · ${breached}`; hlthEl.style.setProperty('color', 'var(--purple)', 'important'); }
}

function formatAccountPhase(a) {
  const type = String(a.challengeType || '').trim();
  const cp = parseInt(a.currentPhase, 10);
  if (type === 'Funded') return 'Funded';
  if (type === 'Instant') return 'Instant';
  if (Number.isFinite(cp) && cp > 0) return 'Phase ' + cp;
  const m = String(a.phase || '').match(/phase\s*(\d+)/i);
...
function switchCapView(view) {
  ...
  const barEl    = document.getElementById('port-cap-bar');
  ...
  const count = view === 'funded' ? (window._portFundedCount || 0) : (window._portChallengeCount || 0);
  capEl.textContent = '$' + val.toFixed(2);
  const capColor = view === 'funded' ? 'var(--green)' : 'var(--purple)';
  capEl.style.setProperty('color', capColor, 'important');
  if (subEl) {
    const label = view === 'funded' ? 'funded' : 'challenge';
    const acctWord = count !== 1 ? 'accounts' : 'account';
    subEl.textContent = `across ${count} ${label} ${acctWord}`;
  }
  if (labelEl) labelEl.textContent = view === 'funded' ? 'Funded Capital' : 'Challenge Capital';
  if (barEl) barEl.style.background = view === 'funded' ? 'var(--green)' : 'var(--purple)';
...
function renderAcctList() {
  const allRow = `
    <div onclick="selectAcct(-1)" style="display:flex;align-items:center;gap:10px;padding:10px 14px;cursor:pointer;transition:background .12s;background:${selectedAcct===-1?'var(--ac-10)':'transparent'};border-left:2px solid ${selectedAcct===-1?'var(--purple)':'transparent'}" onmouseenter="this.style.background='var(--ac-07)'" onmouseleave="this.style.background='${selectedAcct===-1?'var(--ac-10)':'transparent'}'">
      ...
      <div style="flex:1;min-width:0">
        <div style="font-size:13.5px;font-weight:600;color:${selectedAcct===-1?'var(--text)':'var(--text2)'}">All Accounts</div>
        <div style="font-size:11px;color:var(--text3);font-family:var(--font-mono);margin-top:1px">Combined view</div>
      </div>
  ...
  function buildRow({ a, i, status, pnl }) {
    ...
    const phaseLabel = formatAccountPhase(a);
    return `
    <div style="display:flex;align-items:center;position:relative;background:${isSelected?'var(--ac-10)':'transparent'};border-left:2px solid ${isSelected?'var(--purple)':'transparent'};${status!=='active'?'opacity:.7':''}transition:background .12s;" onmouseenter="this.style.background='var(--ac-07)'" onmouseleave="this.style.background='${isSelected?'var(--ac-10)':'transparent'}'">
      <div onclick="selectAcct(${i})" style="display:flex;align-items:center;gap:10px;padding:10px 14px;cursor:pointer;flex:1;min-width:0">
        <div style="position:relative;flex-shrink:0">
          ${firmLogoHTML(a.firm, 28, 7, a.avatar)}
        ...
        <div style="flex:1;min-width:0;display:flex;align-items:center;gap:8px">
          <div style="font-size:13.5px;font-weight:600;color:${isSelected?'var(--text)':'var(--text2)'};white-space:nowrap;overflow:hidden;text-overflow:ellipsis">${a.firm}</div>
          <div style="font-size:11px;color:var(--text3);font-family:var(--font-mono);white-space:nowrap">${phaseLabel}</div>
        </div>
        <div style="display:flex;flex-direction:column;align-items:flex-end;gap:3px;flex-shrink:0;margin-left:8px">
          <span style="font-size:13px;font-weight:700;font-family:var(--font-mono);color:var(--text)">$${(a.startBal||0).toLocaleString()}</span>
  ...
  const sectionLabel = (label, count) =>
    `<div style="padding:6px 14px 4px;font-size:26px;font-weight:600;letter-spacing:.12em;color:var(--text3);font-family:var(--font-mono);text-transform:uppercase;display:flex;align-items:center;gap:10px">
      <span>${label}</span>
      <span style="flex:1;height:1px;background:var(--border)"></span>
      <span style="background:var(--bg5);color:var(--text3);border-radius:10px;padding:3px 8px;font-size:18px">${count}</span>
    </div>`;
...
function selectAcct(i) {
  selectedAcct = i;
  if (window._tradeLogDrilldownActive) clearTradeLogTransientFilters(true);
  if (i === -1) {
...
function toggleAcctDropdown() {
  ...
  const animateDropdown = (el, open) => {
    if (!el) return;
    if (el._hideTimer) {
      clearTimeout(el._hideTimer);
      el._hideTimer = null;
    }
    el.style.transition = 'opacity .2s cubic-bezier(.22,1,.36,1), transform .2s cubic-bezier(.22,1,.36,1)';
    ...
    if (open) {
      ...
      el._hideTimer = setTimeout(() => {
        if (!acctOpen) el.style.display = 'none';
      }, 210);
    }
  };
  
  animateDropdown(dd, acctOpen);
  animateDropdown(ddDashboard, acctOpen);
  ...
  const rowsHtml = acctStatuses.map(({ a, i, status, pnl }) => {
    ...
    <div style="display:flex;align-items:center;border-radius:8px;background:${isSelected?'var(--ac-12)':'transparent'};transition:background .12s;position:relative" onmouseenter="this.style.background='var(--ac-08)'" onmouseleave="this.style.background='${isSelected?'var(--ac-12)':'transparent'}'">
      <div onclick="selectAcctFromTopbar(${i})" style="display:flex;align-items:center;gap:10px;padding:9px 10px;cursor:pointer;flex:1;min-width:0">
        <div style="position:relative;flex-shrink:0">
          ${firmLogoHTML(a.firm, 30, 8, a.avatar)}
        ...
        <div style="flex:1;min-width:0">
          ...
          <div style="font-size:9.5px;color:var(--text3);font-family:var(--font-mono);margin-top:1px">${formatAccountPhase(a)} · ${a.balance}</div>
        </div>
...
function updateTopbarAcctLabel() {
  ...
  if (selectedAcct === -1) {
    ...
    if (subEl) { subEl.textContent = 'Portfolio view'; subEl.style.color = 'var(--purple)'; }
  } else {
    const a = ACCOUNTS[selectedAcct];
...
document.addEventListener('DOMContentLoaded', () => {
  try {
    ...
    const accentColor = getComputedStyle(root).getPropertyValue('--accent').trim() || '#995dff';
...
function populateDashboard() {
  ...
  // ── Avg RR ──
  const tradesWithSL = T.filter(t => {
    const isImported = String(t.setup || '').toLowerCase() === 'imported';
    return !isImported && Number.isFinite(t.sl) && Number.isFinite(t.entry) && Number.isFinite(t.exit) && (
      (t.dir === 'long' && t.sl < t.entry) ||
      (t.dir === 'short' && t.sl > t.entry)
    );
  });
  const rrVals = tradesWithSL
    .map(t => {
  ...
  const avgRR = rrVals.length
    ? (rrVals.reduce((s, v) => s + v, 0) / rrVals.length).toFixed(2)
    : '—';
  ...
  if (selectedAcct === -1) {
    ...
    set('banner-acct-size', '$' + ACCOUNT_SIZE.toFixed(2), 'var(--purple)');
  }
  const acctPnlBadge = $('acct-pnl-pct');
  if (acctPnlBadge) {
    set('acct-pnl-pct', (netPnl >= 0 ? '+' : '') + pnlPct + '%');
    acctPnlBadge.className = 'badge ' + (netPnl >= 0 ? 'badge-green' : 'badge-red');
  }
  ...
  // ── Win rate card ──
  // Keep thresholds aligned with wr-ring animator in account.js
  const wrColor = winRate >= 60 ? 'var(--green)' : winRate >= 45 ? 'var(--amber)' : 'var(--red)';
  const wrText  = winRate >= 60 ? 'GREAT' : winRate >= 45 ? 'GOOD' : 'NEEDS WORK';
  ...
  const tlGrossEl = $('tl-gross');
  if (tlGrossEl) tlGrossEl.style.setProperty('color', grossPnl >= 0 ? 'var(--green)' : 'var(--red)', 'important');
  set('tl-comm',  '-$' + totalComm.toFixed(2));
  set('tl-net',   (netPnl >= 0 ? '+$' : '-$') + Math.abs(netPnl).toFixed(2), netPnl >= 0 ? 'var(--green)' : 'var(--red)');
  set('tl-net-hover', (netPnl >= 0 ? '+$' : '-$') + Math.abs(netPnl).toFixed(2), netPnl >= 0 ? 'var(--green)' : 'var(--red)');
  ...
  const tlWinrateEl = $('tl-winrate');
  if (tlWinrateEl) {
    tlWinrateEl.className = 'stat-val';
    tlWinrateEl.style.setProperty('color', tlWrColor, 'important');
  }
  ...
  if (prRemaining) prRemaining.innerHTML = `${fmtDollar(profitRemaining)} <span style="font-size:11px;color:#1f8f53!important;font-weight:400!important">(${profitRemPct}%)</span>`;
  ...
  if (profitLimitEl) profitLimitEl.innerHTML = isFunded
    ...
    : `${fmtDollar(PROFIT_TARGET)} <span style="font-size:11px;color:#7b3bcc!important;font-weight:400!important">(${profitTargetPct}%)</span>`;

  // ── Max Loss card ──
  const MAX_LOSS_LIMIT = ACCOUNT_SIZE * (maxDrawdownPct / 100);
  const mlLimitEl = $('maxloss-limit-amt');
  if (mlLimitEl) mlLimitEl.innerHTML = `${fmtDollar(MAX_LOSS_LIMIT)} <span style="font-size:11px;color:#7b3bcc!important;font-weight:400!important">(${maxDrawdownPct}%)</span>`;
  ...
  if (mlRemaining) mlRemaining.innerHTML = `${fmtDollar(remaining)} <span style="font-size:11px;color:#c53030!important;font-weight:400!important">(${remPct}%)</span>`;
  ...
  if (dlLimitEl) dlLimitEl.innerHTML = `${fmtDollar(DAILY_LOSS_LIMIT)} <span style="font-size:11px;color:#7b3bcc!important;font-weight:400!important">(${dailyLossPct}%)</span>`;

  const dlRemEl = $('dailyloss-remaining');
  if (dlRemEl) dlRemEl.innerHTML = `${fmtDollar(dlRemaining)} <span style="font-size:11px;color:#c53030!important;font-weight:400!important">(${dlRemPct}%)</span>`;
...
function toggleTheme() {
  ...
  const cur = cl.contains('light') ? 'light' : 'dark';
...
function updateAccountPhotoPreview(photoData) {
  const img = $('acm-avatar-img');
  const fallback = $('acm-avatar-fallback');
  const removeBtn = $('acm-remove-photo-btn');
  if (!img || !fallback || !removeBtn) return;
...
function updateAccountAvatarFallback() {
  const fallback = $('acm-avatar-fallback');
  const firmInput = $('acm-firm');
  if (!fallback) return;
  const raw = (firmInput && firmInput.value ? firmInput.value : '').trim();
  if (!raw) {
  ...
  else initials = (raw[0] || '') + (raw[1] || '');
  fallback.textContent = initials.toUpperCase();
}

function triggerAccountPhotoUpload() {
  const input = $('acm-photo-input');
...
function handleAccountPhotoUpload(input) {
  ...
  reader.onload = function(e) {
    _acmPhotoData = e.target.result;
    updateAccountPhotoPreview(_acmPhotoData);
    if (typeof showToast === 'function') showToast('Account photo updated', 'success', '', 1500);
  };
...
function updatePhaseFields() {
  try {
    ...
    // Show single profit target only for 1 Phase. Multi-phase uses the dedicated phase targets section.
    if (profitCol) {
      const showSingleProfit = phaseType === '1 Phase';
      profitCol.style.display = showSingleProfit ? '' : 'none';
    }
    
    // ──────────────────────────────────────────────────────────
    // Current Phase field: Show & ENABLE EDITING only for 2 Phase & 3 Phase
    // ──────────────────────────────────────────────────────────
    if (currentPhaseInput) {
      const isMultiPhase = phaseType === '2 Phase' || phaseType === '3 Phase';
      
      // Show/hide the field
      currentPhaseInput.parentElement.style.display = isMultiPhase ? '' : 'none';
      
      // Enable/disable editing
      if (isMultiPhase) {
        // Show and make editable
        currentPhaseInput.disabled = false;
        currentPhaseInput.style.opacity = '1';
        currentPhaseInput.style.cursor = 'text';
        currentPhaseInput.style.pointerEvents = 'auto';
        if (currentPhaseInput.value === '' || currentPhaseInput.value === '0') {
          currentPhaseInput.value = '1';
        }
        // Set max value based on phase type
        currentPhaseInput.max = phaseType === '3 Phase' ? '3' : '2';
      } else {
        // Hide and disable
        currentPhaseInput.disabled = true;
        currentPhaseInput.value = '1';
        currentPhaseInput.style.opacity = '0.5';
        currentPhaseInput.style.cursor = 'not-allowed';
        currentPhaseInput.style.pointerEvents = 'none';
      }
    }
...
function checkAndCreateNextPhase(accountName) {
  try {
    ...
    if (!account || !account.challengeType || account.challengeType === 'Instant' || account.challengeType === 'Funded') return;
    ...
    // Get the profit target for current phase
    const currentPhaseNum = account.currentPhase || 1;
    const profitTargetPercent = account.phaseTargets && account.phaseTargets[currentPhaseNum] 
      ? account.phaseTargets[currentPhaseNum] 
      : (account.profitTarget || 8);
    const profitTargetAmount = (account.startBal * profitTargetPercent) / 100;
    
    // Extract total phases from challenge type (e.g., "2 Phase" → 2, "3 Phase" → 3)
    let totalPhases = 1;
    if (account.challengeType.includes('Phase')) {
      const match = account.challengeType.match(/^(\d+)\s+Phase/);
      if (match) totalPhases = parseInt(match[1]) || 1;
    }
    
    // Check if profit target reached and next phase doesn't exist
    if (totalPnL >= profitTargetAmount && currentPhaseNum < totalPhases) {
      // Check if next phase account already exists
      const nextPhaseNum = currentPhaseNum + 1;
      const nextPhaseKey = `${account.firm} - Phase ${nextPhaseNum}`;
      const nextPhaseExists = ACCOUNTS.some(a => a.key === nextPhaseKey.replace(/\s+/g, ' ').trim());
      ...
      if (!nextPhaseExists) {
        ...
        const nextPhaseAccount = {
          ...
          firm: account.firm,
          phase: nextPhaseKey,
          key: nextPhaseKey.replace(/\s+/g, ' ').trim(),
          ...
          notes: account.notes ? account.notes + ` [Auto-created from Phase ${currentPhaseNum}]` : `[Auto-created from Phase ${currentPhaseNum}]`,
...
// ═══════════════════════════════════════════════════════════════
// ── CONSISTENCY RULES & PAYOUT ELIGIBILITY CALCULATIONS ──
// ═══════════════════════════════════════════════════════════════

/**
 * Calculate consistency metrics for an account
 * @param {string} accountName - Account identifier
 * @returns {object} Consistency metrics including win days, loss days, etc.
 */
function calculateConsistencyRules(accountName) {
  try {
    const account = ACCOUNTS.find(a => (a.key || a.phase) === accountName || a.phase === accountName);
    if (!account) return null;

    const trades = TRADES.filter(t => t.account === accountName);
    if (trades.length === 0) {
      return {
        totalTrades: 0,
        totalWins: 0,
    ...
    const daysArray = Object.keys(tradesByDate).sort();
    const totalDays = daysArray.length;

    // Calculate daily P&L
    let winDays = 0, lossDays = 0, breakEvenDays = 0;
    let totalWins = 0, totalLosses = 0, totalWinTrades = 0, totalLossTrades = 0;
    let largestWin = 0, largestLoss = 0;
    let peakEquity = account.startBal;
    let maxDrawdown = 0;
    let currentEquity = account.startBal;
    let consecLossDays = 0, maxConsecLossDays = 0;
    ...
    daysArray.forEach(date => {
      ...
      dayTrades.forEach(t => {
        if (t.pnl && t.pnl > 0) {
          ...
          totalLossTrades++;
          largestLoss = Math.max(largestLoss, Math.abs(t.pnl));
        }
      });
      ...
      if (dayPnL > 0) {
        ...
        lossDays++;
        consecLossDays++;
        maxConsecLossDays = Math.max(maxConsecLossDays, consecLossDays);
      } else {
        breakEvenDays++;
        consecLossDays = 0;
      }
    ...
    const avgWin = totalWinTrades > 0 ? totalWins / totalWinTrades : 0;
    const avgLoss = totalLossTrades > 0 ? totalLosses / totalLossTrades : 0;
    ...
    console.warn('calculateConsistencyRules error:', e);
    return null;
  }
}

/**
 * Check if account is eligible for payout based on rules
 * @param {string} accountName - Account identifier
 * @returns {object} Eligibility status and reasons
 */
function checkPayoutEligibility(accountName) {
  try {
    const account = ACCOUNTS.find(a => (a.key || a.phase) === accountName || a.phase === accountName);
    if (!account) return { eligible: false, reasons: ['Account not found'] };

    // Instant and Funded accounts have no restrictions
    if (account.challengeType === 'Instant' || account.challengeType === 'Funded') {
      return { 
        eligible: true, 
        accountType: account.challengeType,
        reasons: [`${account.challengeType} account - No restrictions`] 
      };
    }

    const consistency = calculateConsistencyRules(accountName);
    if (!consistency) {
      return { eligible: false, reasons: ['Unable to calculate consistency metrics'] };
    }
    ...
    // Check profit target
    const profitTargetAmount = (account.startBal * (account.profitTarget || 8)) / 100;
    if (consistency.totalPnL >= profitTargetAmount) {
      reasons.push(`✓ Profit target reached: $${consistency.totalPnL.toFixed(2)} / $${profitTargetAmount.toFixed(2)}`);
    } else {
      reasons.push(`✗ Profit target NOT met: $${consistency.totalPnL.toFixed(2)} / $${profitTargetAmount.toFixed(2)}`);
      eligible = false;
    }

    // Check max drawdown
    const maxDDLimit = account.maxDrawdown || 8;
    if (consistency.maxDrawdown <= maxDDLimit) {
      reasons.push(`✓ Max drawdown within limit: ${consistency.maxDrawdown.toFixed(2)}% / ${maxDDLimit}%`);
    } else {
      reasons.push(`✗ Max drawdown exceeded: ${consistency.maxDrawdown.toFixed(2)}% / ${maxDDLimit}%`);
      eligible = false;
    }

    // Check daily loss limit
    const dailyLossLimit = account.dailyLoss || 4;
    const maxDailyLoss = Math.max(...Object.values(
      TRADES.filter(t => t.account === accountName)
        .reduce((acc, t) => {
          acc[t.date] = (acc[t.date] || 0) + t.pnl;
          return acc;
        }, {})
    ).filter(v => v < 0).map(v => Math.abs(v)), 0);
    
    if (maxDailyLoss <= (account.startBal * dailyLossLimit / 100)) {
      reasons.push(`✓ Daily loss limit respected: $${maxDailyLoss.toFixed(2)} / $${(account.startBal * dailyLossLimit / 100).toFixed(2)}`);
    } else {
      reasons.push(`✗ Daily loss exceeded: $${maxDailyLoss.toFixed(2)} / $${(account.startBal * dailyLossLimit / 100).toFixed(2)}`);
      eligible = false;
    }

    // Additional consistency metrics
    reasons.push(`\nConsistency Metrics:`);
    reasons.push(`• Win Rate: ${consistency.winRate.toFixed(1)}%`);
    reasons.push(`• Win Days: ${consistency.winDays} / ${consistency.totalDays}`);
    reasons.push(`• Profit Factor: ${consistency.profitFactor.toFixed(2)}`);
    reasons.push(`• Risk/Reward: ${consistency.riskRewardRatio.toFixed(2)}`);
    ...
    console.warn('checkPayoutEligibility error:', e);
    return { eligible: false, reasons: [`Error: ${e.message}`] };
  }
}

/**
 * Check if account can clear to next phase
 * @param {string} accountName - Account identifier
 * @returns {object} Status and requirements
 */
function checkPhaseClearing(accountName) {
  try {
    const account = ACCOUNTS.find(a => (a.key || a.phase) === accountName || a.phase === accountName);
    if (!account) return { canClear: false, reasons: ['Account not found'] };

    // Only applicable to multi-phase challenges
    if (account.challengeType === 'Instant' || account.challengeType === 'Funded') {
      return { 
        canClear: false, 
        accountType: account.challengeType,
        reasons: [`${account.challengeType} accounts don't have phase progression`] 
      };
    }
    ...
    const currentPhase = account.currentPhase || 1;
    const reasons = [];

    // If already at final phase, check if can reach Funded
    if (currentPhase === totalPhases) {
      const eligibility = checkPayoutEligibility(accountName);
      const canReachFunded = eligibility.eligible;
      reasons.push(`Currently on Phase ${currentPhase} (Final Phase)`);
      if (canReachFunded) {
        reasons.push(`✓ Ready to reach Funded status`);
      } else {
        reasons.push(`✗ Not yet ready for Funded:`);
        eligibility.reasons.forEach(r => reasons.push(`  ${r}`));
      }
      return { canClear: canReachFunded, isFinalPhase: true, reasons };
    } else {
      // Check if can progress to next phase
      const eligibility = checkPayoutEligibility(accountName);
      const nextPhase = currentPhase + 1;
      reasons.push(`Currently on Phase ${currentPhase} of ${totalPhases}`);
      
      if (eligibility.eligible) {
        reasons.push(`✓ Ready to progress to Phase ${nextPhase}`);
      } else {
        reasons.push(`✗ Not yet ready for Phase ${nextPhase}:`);
        eligibility.reasons.forEach(r => reasons.push(`  ${r}`));
      }
      return { canClear: eligibility.eligible, nextPhase, reasons };
    }
  } catch (e) {
    console.warn('checkPhaseClearing error:', e);
    return { canClear: false, reasons: [`Error: ${e.message}`] };
  }
}
...
function togglePasswordVisibility(fieldId) {
  ...
  const iconId = fieldId === 'auth-password' ? 'auth-password-icon' : 'auth-password-confirm-icon';
  ...
  if (!icon) return;
  if (show) {
    icon.innerHTML = '<path d="M1 12s4-8 11-8 11 8 11 8-4 8-11 8-11-8-11-8z"></path><path d="M17.94 17.94A10.08 10.08 0 0 1 12 20c-7 0-11-8-11-8a21.16 21.16 0 0 1 5.06-7.94"></path><line x1="1" y1="1" x2="23" y2="23"></line>';
  } else {
    ...
    <div class="acct-page-card status-${status}"
      ...
      <div style="display:flex;align-items:flex-start;justify-content:space-between;margin-bottom:16px;position:relative" onclick="${acctSelectMode ? `toggleAccountSelection(${a.id})` : `openEditAccountModal(${a.id})`}">
        <div style="display:flex;align-items:flex-start;gap:10px;flex:1;min-width:0">
          <div style="flex-shrink:0">${firmLogoHTML(a.firm, 34, 9, a.avatar)}</div>
          <div style="flex:1;min-width:0">
          <div style="display:flex;align-items:center;gap:7px;flex-wrap:wrap;margin-bottom:5px">
            ...
            <div style="font-family:var(--font-display);font-size:17px;font-weight:600;color:var(--text)">${a.firm}</div>
            ${status === 'breached' ? (() => {
          ...
          <div style="font-size:11px;color:var(--text3);font-family:var(--font-mono)">${formatAccountPhase(a)}</div>
          </div>
        </div>
    ...
    <div style="background:var(--bg2);border:1px solid var(--border);border-radius:16px;padding:20px;opacity:.6;position:relative">
      <div style="display:flex;align-items:flex-start;justify-content:space-between;margin-bottom:12px">
        <div style="flex:1">
          ...
          <div style="font-size:11px;color:var(--text4);font-family:var(--font-mono);margin-top:2px">${formatAccountPhase(a)}</div>
        </div>
